...
export class FractionComponent implements OnInit {
  AMOUNT = 50000;
  COINS = [100000, 50000, 20000, 10000, 5000, 1000, 500, 100, 50];
  coincount = [0, 0, 0, 0, 0, 0, 0, 0, 0];

  constructor() {
  }

  ngOnInit() {
    function computeChange(coins: any, amount: any) {
      let i = 0; let creminder = this.AMOUNT; let ccoin: number;

      while (i < this.COINS.length) {
        while (this.COINS[i] <= creminder) {
          creminder = creminder - this.COINS[i];
          ccoin = this.coincount[i];
          ccoin += 1;
          this.coincount[i] = ccoin;
    ...
    function makeChange() {
      ...
      // how many of the corresponding value in this.COINS to give. The
      // total value should equal this.AMOUNT.
      let change = computeChange(this.COINS, this.AMOUNT);
      ...
      //    1        5
      //    3        1
      // TOTAL this.AMOUNT: 68 (total is correct)
      //
      ...
      if (change.length > this.COINS.length) {
        return 'Error: the function computeChange returned an array ' +
          'longer than the length (' + this.COINS.length + ') of this.COINS.';
      }
      if (change.length < this.COINS.length) {
        return 'Error: the function computeChange returned an array ' +
          'shorter than the length (' + this.COINS.length + ') of this.COINS.';
      }
      let output = '<pre>NUMBER   VALUE\n';
      let sum = 0;
      for (const item of this.sum) {
        sum += change[item] * this.COINS[item];
        const n = change[item].toString();
        const a = this.COINS[item].toString();
        output += rightJustify(n, 4) + rightJustify(a, 9) + '\n';
      }
      output += 'TOTAL this.AMOUNT: ' + sum + ' (total is ';
      output += (sum === this.AMOUNT ? 'correct' :
        'incorrect, should be ' + this.AMOUNT) + ')\n';
      return output;
    }
